...
import cn.tianyu.client.viamcp.utils.NettyUtil;
import java.security.Key;
import net.minecraft.util.CryptManager;
import javax.crypto.SecretKey;
import com.viaversion.viaversion.api.connection.UserConnection;
import cn.tianyu.client.viamcp.handler.MCPDecodeHandler;
import cn.tianyu.client.viamcp.handler.MCPEncodeHandler;
    ...
    static {
        ...
        CLIENT_NIO_EVENTLOOP = new LazyLoadBase<NioEventLoopGroup>() {
            ...
            protected NioEventLoopGroup load() {
                return new NioEventLoopGroup(0, new ThreadFactoryBuilder().setNameFormat("Netty TYClient IO #%d").setDaemon(true).build());
            }
        };
        field_181125_e = new LazyLoadBase<EpollEventLoopGroup>() {
            @Override
            protected EpollEventLoopGroup load() {
                return new EpollEventLoopGroup(0, new ThreadFactoryBuilder().setNameFormat("Netty Epoll TYClient IO #%d").setDaemon(true).build());
            }
        };
        CLIENT_LOCAL_EVENTLOOP = new LazyLoadBase<LocalEventLoopGroup>() {
            @Override
            protected LocalEventLoopGroup load() {
                return new LocalEventLoopGroup(0, new ThreadFactoryBuilder().setNameFormat("Netty Local TYClient IO #%d").setDaemon(true).build());
            }
        };
    }
