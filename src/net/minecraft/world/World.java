...
import cn.tianyu.client.viamcp.utils.FixedSoundEngine;
...
public abstract class World implements IBlockAccess
{
            ...
            for (int l2 = k; l2 < l; ++l2) {
                for (int i3 = i2; i3 < j2; ++i3) {
                    ...
                    if (block.getMaterial() == materialIn) {
                        final double d0 = l2 + 1 - BlockLiquid.getLiquidHeightPercent(iblockstate.getValue(BlockLiquid.LEVEL));
                        if (l >= d0) {
                            flag = true;
        ...
        for (int k2 = i; k2 < j; ++k2) {
            for (int l2 = k; l2 < l; ++l2) {
                for (int i3 = i2; i3 < j2; ++i3) {
                    ...
                    final Block block = iblockstate.getBlock();
                    if (block.getMaterial() == materialIn) {
                        final int j3 = iblockstate.getValue(BlockLiquid.LEVEL);
                        double d0 = l2 + 1;
                        if (j3 < 8) {
                            d0 = l2 + 1 - j3 / 8.0;
                        }
    ...
    public <T extends Entity> List<T> getEntities(final Class<? extends T> entityType, final com.google.common.base.Predicate<? super T> filter) {
    ...
    public <T extends Entity> List<T> getPlayers(final Class<? extends T> playerType, final com.google.common.base.Predicate<? super T> filter) {
        final List<T> list = Lists.newArrayList();
