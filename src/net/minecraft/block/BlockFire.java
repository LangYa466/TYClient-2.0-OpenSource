...
public class BlockFire extends Block
{
    ...
    public IBlockState getActualState(final IBlockState state, final IBlockAccess worldIn, final BlockPos pos) {
        ...
        if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.down()) && !Blocks.fire.canCatchFire(worldIn, pos.down())) {
            ...
            return state.withProperty(BlockFire.NORTH, this.canCatchFire(worldIn, pos.north())).withProperty(BlockFire.EAST, this.canCatchFire(worldIn, pos.east())).withProperty(BlockFire.SOUTH, this.canCatchFire(worldIn, pos.south())).withProperty(BlockFire.WEST, this.canCatchFire(worldIn, pos.west())).withProperty(BlockFire.UPPER, l).withProperty(BlockFire.FLIP, flag2).withProperty(BlockFire.ALT, flag);
        }
    ...
    public void updateTick(final World worldIn, final BlockPos pos, IBlockState state, final Random rand) {
        if (worldIn.getGameRules().getBoolean("doFireTick")) {
            ...
            if (!flag && worldIn.isRaining() && this.canDie(worldIn, pos)) {
                ...
                final int i = state.getValue(BlockFire.AGE);
                if (i < 15) {
                            ...
                            if (k != 0 || i2 != 0 || l != 0) {
                                ...
                                if (k2 > 0) {
                                    ...
                                    if (l2 > 0 && rand.nextInt(j2) <= l2 && (!worldIn.isRaining() || !this.canDie(worldIn, blockpos))) {
                                        ...
                                        worldIn.setBlockState(blockpos, state.withProperty(BlockFire.AGE, i3), 3);
                                    }
                                }
                            }
                        }
    ...
    private void catchOnFire(final World worldIn, final BlockPos pos, final int chance, final Random random, final int age) {
        ...
        if (random.nextInt(chance) < i) {
            ...
            if (random.nextInt(age + 10) < 5 && !worldIn.canLightningStrike(pos)) {
                ...
                worldIn.setBlockState(pos, this.getDefaultState().withProperty(BlockFire.AGE, j), 3);
            }
            else {
                worldIn.setBlockToAir(pos);
            }
        ...
        if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.down()) && !Blocks.fire.canCatchFire(worldIn, pos.down())) {
            if (Blocks.fire.canCatchFire(worldIn, pos.west())) {
                for (int j = 0; j < 2; ++j) {
                    ...
                    worldIn.spawnParticle(EnumParticleTypes.SMOKE_LARGE, d3, d4, d5, 0.0, 0.0, 0.0);
                }
            }
            if (Blocks.fire.canCatchFire(worldIn, pos.east())) {
                for (int k = 0; k < 2; ++k) {
                    ...
                    worldIn.spawnParticle(EnumParticleTypes.SMOKE_LARGE, d6, d7, d8, 0.0, 0.0, 0.0);
                }
            }
            if (Blocks.fire.canCatchFire(worldIn, pos.north())) {
                for (int l = 0; l < 2; ++l) {
                    ...
                    worldIn.spawnParticle(EnumParticleTypes.SMOKE_LARGE, d9, d10, d11, 0.0, 0.0, 0.0);
                }
            }
            if (Blocks.fire.canCatchFire(worldIn, pos.south())) {
                for (int i1 = 0; i1 < 2; ++i1) {
                    ...
                    worldIn.spawnParticle(EnumParticleTypes.SMOKE_LARGE, d12, d13, d14, 0.0, 0.0, 0.0);
                }
            }
            if (Blocks.fire.canCatchFire(worldIn, pos.up())) {
                for (int j2 = 0; j2 < 2; ++j2) {
                    ...
                    worldIn.spawnParticle(EnumParticleTypes.SMOKE_LARGE, d15, d16, d17, 0.0, 0.0, 0.0);
                }
            }
        }
        else {
            for (int m = 0; m < 3; ++m) {
                ...
                worldIn.spawnParticle(EnumParticleTypes.SMOKE_LARGE, d18, d19, d20, 0.0, 0.0, 0.0);
            }
        }
    }
    ...
    public int getMetaFromState(final IBlockState state) {
        return state.getValue(BlockFire.AGE);
    }
