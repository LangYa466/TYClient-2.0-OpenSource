...
import cn.tianyu.client.IRC.TYClientIRC;
...
public class GuiIngame extends Gui
{
    ...
    public static void NMSL() {
        if (Minecraft.thePlayer != null) {
            Minecraft.getMinecraft().ingameGUI.getChatGUI().printChatMessage(new ChatComponentText("[TYClient]对着模块按右键查看中文翻译~"));
            final GuiNewChat chatGUI = Minecraft.getMinecraft().ingameGUI.getChatGUI();
            final StringBuilder append = new StringBuilder().append("[TYClient]当前客户端版本:");
            final TYClient instance = TYClient.instance;
            chatGUI.printChatMessage(new ChatComponentText(append.append("2.0").toString()));
            Minecraft.getMinecraft().ingameGUI.getChatGUI().printChatMessage(new ChatComponentText("[TYClient]客户端官网:http://150.138.78.137:9108/"));
            Minecraft.getMinecraft().ingameGUI.getChatGUI().printChatMessage(new ChatComponentText("[TYClient]客户端官方QQ群:471873571"));
            Minecraft.getMinecraft().ingameGUI.getChatGUI().printChatMessage(new ChatComponentText("[TYClient-IRC].i + 你要发送的内容发送IRC消息（你需要先发送一条消息开启IRC）"));
        }
    }
    ...
    public void renderGameOverlay(final float partialTicks) {
        ...
        if (!Minecraft.thePlayer.isPotionActive(Potion.confusion)) {
            ...
            final float prevTimeInPortal = Minecraft.thePlayer.prevTimeInPortal;
            final Minecraft mc5 = this.mc;
            final float timeInPortal = Minecraft.thePlayer.timeInPortal;
            final Minecraft mc6 = this.mc;
            final float f = prevTimeInPortal + (timeInPortal - Minecraft.thePlayer.prevTimeInPortal) * partialTicks;
            if (f > 0.0f) {
        ...
        if (this.mc.playerController.isSpectator()) {
            ...
            final TYClient instance = TYClient.instance;
            TYClient.modManager.getEnableMods().forEach(Mod::draw);
            final TYClient instance2 = TYClient.instance;
            if (!TYClient.modManager.getByClass(BetterTab.class).isEnable()) {
                this.renderTooltip(scaledresolution, partialTicks);
            }
            final TYClient instance3 = TYClient.instance;
            if (TYClient.modManager.getByClass(Effect.class).isEnable()) {
                Effect.onRenderGameOverlay();
            }
            FontManager.F48.drawString2("TYClient", 0.0, 0.0, new Color(255, 255, 255).getRGB());
            if (Minecraft.thePlayer != null && GuiIngame.L) {
                NMSL();
                GuiIngame.L = false;
            }
            ...
            final TYClient instance4 = TYClient.instance;
            if (!TYClient.modManager.getByClass(Effect.class).isEnable()) {}
        }
        ...
        if (Minecraft.thePlayer.getSleepTimer() > 0) {
            ...
            final int j1 = Minecraft.thePlayer.getSleepTimer();
            float f2 = j1 / 100.0f;
            if (f2 > 1.0f) {
                f2 = 1.0f - (j1 - 100) / 10.0f;
            }
            final int k = (int)(220.0f * f2) << 24 | 0x101020;
            Gui.drawRect(0, 0, width, height, k);
            GlStateManager.enableAlpha();
            GlStateManager.enableDepth();
            this.mc.mcProfiler.endSection();
        }
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
        final int k2 = width / 2 - 91;
        ...
        if (this.recordPlayingUpFor > 0) {
            ...
            if (l1 > 8) {
                ...
                if (this.recordIsPlaying) {
                    i = (MathHelper.func_181758_c(f3 / 50.0f, 0.7f, 0.6f) & 0xFFFFFF);
                }
                this.getFontRenderer().drawString(this.recordPlaying, -this.getFontRenderer().getStringWidth(this.recordPlaying) / 2, -4, i + (l1 << 24 & 0xFF000000));
                GlStateManager.disableBlend();
        ...
        final ScorePlayerTeam scoreplayerteam = scoreboard2.getPlayersTeam(Minecraft.thePlayer.getName());
            ...
            Label_1445: {
                if (this.mc.gameSettings.keyBindPlayerList.isKeyDown()) {
                    if (this.mc.isIntegratedServerRunning()) {
                        ...
                        if (Minecraft.thePlayer.sendQueue.getPlayerInfoMap().size() <= 1 && scoreobjective2 == null) {
                            break Label_1445;
                        }
                    }
                    this.overlayPlayerList.updatePlayerList(true);
                    this.overlayPlayerList.renderPlayerlist(width, scoreboard, scoreobjective2);
                    break Label_1476;
                }
            }
    ...
    public void renderExpBar(final ScaledResolution p_175176_1_, final int p_175176_2_) {
        ...
        if (i > 0) {
            final int j = 182;
            final Minecraft mc2 = this.mc;
            final int k = (int)(Minecraft.thePlayer.experience * (j + 1));
        ...
        if (Minecraft.thePlayer.experienceLevel > 0) {
            ...
            final Minecraft mc4 = this.mc;
            final String s = append.append(Minecraft.thePlayer.experienceLevel).toString();
    ...
    public void renderDemo(final ScaledResolution p_175185_1_) {
        ...
        if (Minecraft.theWorld.getTotalWorldTime() >= 120500L) {
            ...
            final String translateKey = "demo.remainingTime";
            final Object[] parameters = { null };
            final int n = 0;
            final long n2 = 120500L;
            final Minecraft mc2 = this.mc;
            parameters[n] = StringUtils.ticksToElapsedTime((int)(n2 - Minecraft.theWorld.getTotalWorldTime()));
            s = I18n.format(translateKey, parameters);
        }
    ...
    protected boolean showCrosshair() {
        if (this.mc.gameSettings.showDebugInfo) {
            final Minecraft mc = this.mc;
            if (!Minecraft.thePlayer.hasReducedDebug() && !this.mc.gameSettings.reducedDebugInfo) {
                return false;
            }
        }
        if (!this.mc.playerController.isSpectator()) {
        ...
        if (this.mc.objectMouseOver != null && this.mc.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
            ...
            final Minecraft mc2 = this.mc;
            if (Minecraft.theWorld.getTileEntity(blockpos) instanceof IInventory) {
                return true;
            }
        }
        return false;
    ...
    private void renderScoreboard(final ScoreObjective p_180475_1_, final ScaledResolution p_180475_2_) {
        ...
        if (list.size() > 15) {
            collection = (Collection<Score>)Lists.newArrayList((Iterable<?>)Iterables.skip((Iterable<?>)list, collection.size() - 15));
        }
        else {
            collection = list;
        }
    ...
    private void renderPlayerStats(final ScaledResolution p_180477_1_) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            if (entityplayer.isInsideOfMaterial(Material.water)) {
                final Minecraft mc = this.mc;
                final int l8 = Minecraft.thePlayer.getAir();
    ...
    private void renderVignette(float p_180480_1_, final ScaledResolution p_180480_2_) {
        ...
        final WorldBorder worldborder = worldBorder = Minecraft.theWorld.getWorldBorder();
        final Minecraft mc2 = this.mc;
