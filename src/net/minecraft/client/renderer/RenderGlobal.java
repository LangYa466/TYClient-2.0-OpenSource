...
public class RenderGlobal implements IWorldAccess, IResourceManagerReloadListener
{
    ...
    protected boolean isRenderEntityOutlines() {
        if (this.entityOutlineFramebuffer != null && this.entityOutlineShader != null) {
            ...
            if (Minecraft.thePlayer != null) {
                ...
                return Minecraft.thePlayer.isSpectator() && this.mc.gameSettings.keyBindSpectatorOutlines.isKeyDown();
            }
        }
        return false;
    }
        ...
        if (this.renderEntitiesStartupCounter > 0) {
            ...
            if (this.isRenderEntityOutlines()) {
                ...
                for (int j = 0; j < list.size(); ++j) {
                    ...
                    Label_0585: {
                        Label_0584: {
                            if (entity3.isInRangeToRender3d(d0, d2, d3)) {
                                if (!entity3.ignoreFrustumCheck && !camera.isBoundingBoxInFrustum(entity3.getEntityBoundingBox())) {
                                    final Entity riddenByEntity = entity3.riddenByEntity;
                                    final Minecraft mc = this.mc;
                                    if (riddenByEntity != Minecraft.thePlayer) {
                                        break Label_0584;
                                    }
                                }
            ...
            for (final ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation : this.renderInfos) {
                ...
                if (!classinheritancemultimap.isEmpty()) {
                    for (final Entity entity4 : classinheritancemultimap) {
                        boolean b2 = false;
                        Label_0865: {
                            if (!this.renderManager.shouldRender(entity4, camera, d0, d2, d3)) {
                                final Entity riddenByEntity2 = entity4.riddenByEntity;
                                final Minecraft mc2 = this.mc;
                                if (riddenByEntity2 != Minecraft.thePlayer) {
                                    b2 = false;
                                    break Label_0865;
                                }
                            }
            ...
            for (final DestroyBlockProgress destroyblockprogress : this.damagedBlocks.values()) {
                ...
                if (tileentity4 instanceof TileEntityChest tileentitychest) {
    ...
    public void setupTerrain(final Entity viewEntity, final double partialTicks, ICamera camera, final int frameCount, final boolean playerSpectator) {
        ...
        if (!flag && this.displayListEntitiesDirty) {
            ...
            final Queue<ContainerLocalRenderInformation> queue = Lists.newLinkedList();
            boolean flag2 = this.mc.renderChunksMany;
            if (renderchunk != null) {
                boolean flag3 = false;
                final ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation3 = new ContainerLocalRenderInformation(renderchunk, null, 0);
                ...
                for (int j = -this.renderDistanceChunks; j <= this.renderDistanceChunks; ++j) {
                    for (int k = -this.renderDistanceChunks; k <= this.renderDistanceChunks; ++k) {
                        ...
                        if (renderchunk2 != null && camera.isBoundingBoxInFrustum(renderchunk2.boundingBox)) {
                            ...
                            queue.add(new ContainerLocalRenderInformation(renderchunk2, null, 0));
                        }
                    }
                }
            }
            while (!queue.isEmpty()) {
                ...
                for (final EnumFacing enumfacing3 : EnumFacing.values()) {
                    ...
                    if ((!flag2 || !renderglobal$containerlocalrenderinformation4.setFacing.contains(enumfacing3.getOpposite())) && (!flag2 || enumfacing2 == null || renderchunk3.getCompiledChunk().isVisible(enumfacing2.getOpposite(), enumfacing3)) && renderchunk4 != null && renderchunk4.setFrameIndex(frameCount) && camera.isBoundingBoxInFrustum(renderchunk4.boundingBox)) {
                        final ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation5 = new ContainerLocalRenderInformation(renderchunk4, enumfacing3, renderglobal$containerlocalrenderinformation4.counter + 1);
    ...
    public void renderSky(final float partialTicks, final int pass) {
        ...
        if (Minecraft.theWorld.provider.getDimensionId() == 1) {
            ...
            if (Minecraft.theWorld.provider.isSurfaceWorld()) {
                ...
                final int k = i % 4;
                final int i2 = i / 4 % 2;
                final float f18 = (k) / 4.0f;
                final float f19 = (i2) / 2.0f;
            ...
            if (this.mc.gameSettings.func_181147_e() == 2) {
                ...
                for (int i2 = -256; i2 < 256; i2 += 32) {
                    for (int j2 = -256; j2 < 256; j2 += 32) {
                        worldrenderer.pos(i2, f9, j2 + 32).tex((i2) * 4.8828125E-4f + f10, (j2 + 32) * 4.8828125E-4f + f11).color(f2, f3, f4, 0.8f).endVertex();
                        worldrenderer.pos(i2 + 32, f9, j2 + 32).tex((i2 + 32) * 4.8828125E-4f + f10, (j2 + 32) * 4.8828125E-4f + f11).color(f2, f3, f4, 0.8f).endVertex();
                        worldrenderer.pos(i2 + 32, f9, j2).tex((i2 + 32) * 4.8828125E-4f + f10, (j2) * 4.8828125E-4f + f11).color(f2, f3, f4, 0.8f).endVertex();
                        worldrenderer.pos(i2, f9, j2).tex((i2) * 4.8828125E-4f + f10, (j2) * 4.8828125E-4f + f11).color(f2, f3, f4, 0.8f).endVertex();
                    }
                }
                tessellator.draw();
                GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
    ...
    public void drawSelectionBox(final EntityPlayer player, final MovingObjectPosition movingObjectPositionIn, final int p_72731_3_, final float partialTicks) {
        if (p_72731_3_ == 0 && movingObjectPositionIn.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
            ...
            if (block.getMaterial() != Material.air && this.theWorld.getWorldBorder().contains(blockpos)) {
                ...
                final AxisAlignedBB axisAlignedBB = block.getSelectedBoundingBox(this.theWorld, blockpos).expand(0.0020000000949949026, 0.0020000000949949026, 0.0020000000949949026).offset(-d0, -d2, -d3);
                final TYClient instance = TYClient.instance;
                final Color color = TYClient.modManager.getByClass(SelectBlock.class).isEnable() ? new Color(255, 255, 255, 100) : new Color(0, 0, 0, 100);
                GlStateManager.color(color.getRed() / 255.0f, color.getGreen() / 255.0f, color.getBlue() / 255.0f, color.getAlpha() / 255.0f);
                final TYClient instance2 = TYClient.instance;
                if (TYClient.modManager.getByClass(SelectBlock.class).isEnable()) {
        ...
        switch (sfxType) {
            ...
            case 2002: {
                ...
                for (int l2 = 0; l2 < 100; ++l2) {
                    ...
                    final EntityFX entityfx = this.spawnEntityFX(enumparticletypes.getParticleID(), enumparticletypes.getShouldIgnoreRange(), d25 + d30 * 0.1, d26 + 0.3, d27 + d32 * 0.1, d30, d31, d32);
                    if (entityfx != null) {
            ...
            case 2003: {
                ...
                for (double d36 = 0.0; d36 < 6.283185307179586; d36 += 0.15707963267948966) {
                    this.spawnParticle(EnumParticleTypes.PORTAL, d33 + Math.cos(d36) * 5.0, d34 - 0.4, d35 + Math.sin(d36) * 5.0, Math.cos(d36) * -5.0, 0.0, Math.sin(d36) * -5.0);
                    this.spawnParticle(EnumParticleTypes.PORTAL, d33 + Math.cos(d36) * 5.0, d34 - 0.4, d35 + Math.sin(d36) * 5.0, Math.cos(d36) * -7.0, 0.0, Math.sin(d36) * -7.0);
                }
                return;
            }
            case 2004: {
                for (int k2 = 0; k2 < 20; ++k2) {
                    ...
                    this.theWorld.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, d37, d38, d39, 0.0, 0.0, 0.0);
                    this.theWorld.spawnParticle(EnumParticleTypes.FLAME, d37, d38, d39, 0.0, 0.0, 0.0);
                }
                return;
            }
            case 2005: {
