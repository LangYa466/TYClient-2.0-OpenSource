    ...
    public void rebuildChunk(float x, float y, float z, ChunkCompileTaskGenerator generator) {
        ...
        if (!iblockaccess.extendedLevelsInChunkCache()) {
            ...
            for (BlockPos.MutableBlockPos blockpos$mutableblockpos : BlockPos.getAllInBoxMutable(blockpos, blockpos1)) {
                ...
                int n = j;
                aboolean[n] = aboolean[n] | blockrendererdispatcher.renderBlock(iblockstate, blockpos$mutableblockpos, iblockaccess, worldrenderer);
            }
            for (EnumWorldBlockLayer enumworldblocklayer : EnumWorldBlockLayer.values()) {
                if (aboolean[enumworldblocklayer.ordinal()]) {
                    compiledchunk.setLayerUsed(enumworldblocklayer);
                }
                if (!compiledchunk.isLayerStarted(enumworldblocklayer)) continue;
        ...
        try {
            ...
            HashSet<TileEntity> set1 = Sets.newHashSet(this.field_181056_j);
