...
implements IResourceManagerReloadListener {
    ...
    public void updateRenderer() {
        ...
        float f3 = Minecraft.theWorld.getLightBrightness(new BlockPos(this.mc.getRenderViewEntity()));
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null) {
            if (Minecraft.theWorld != null) {
                ...
                Vec3 vec33 = null;
                float f = 1.0f;
                List<Entity> list = Minecraft.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec31.xCoord * d0, vec31.yCoord * d0, vec31.zCoord * d0).expand(f, f, f), Predicates.and(EntitySelectors.NOT_SPECTATING, new Predicate<Entity>(){
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (TYClient.modManager.getByClass(NoHurtCamera.class).isEnable()) {
            return;
        }
    ...
    public void setupCameraTransform(float partialTicks, int pass) {
        ...
        if ((f1 = Minecraft.thePlayer.prevTimeInPortal + (Minecraft.thePlayer.timeInPortal - Minecraft.thePlayer.prevTimeInPortal) * partialTicks) > 0.0f) {
            int i = 20;
            ...
            if (world != null) {
                ...
                for (int i = 0; i < 256; ++i) {
                    ...
                    if (Minecraft.thePlayer.isPotionActive(Potion.nightVision)) {
                        float f15 = this.getNightVisionBrightness(Minecraft.thePlayer, partialTicks);
                        float f12 = 1.0f / f8;
                        if (f12 > 1.0f / f9) {
                            f12 = 1.0f / f9;
                        }
    ...
    public void func_181560_a(float p_181560_1_, long p_181560_2_) {
        ...
        if (this.mc.inGameHasFocus && flag) {
            ...
            if (this.mc.gameSettings.smoothCamera) {
                ...
                Minecraft.thePlayer.setAngles(f2, f3 * (float)i);
            } else {
                this.smoothCamYaw = 0.0f;
                this.smoothCamPitch = 0.0f;
                Minecraft.thePlayer.setAngles(f2, f3 * (float)i);
    ...
    private boolean isDrawBlockOutline() {
        ...
        if (this.mc.gameSettings.hideGUI) return false;
        boolean bl = true;
        boolean flag = bl;
        if (!flag) return flag;
        ...
        if (this.mc.playerController.getCurrentGameType() == WorldSettings.GameType.SPECTATOR) {
            ...
            if (!(Minecraft.theWorld.getTileEntity(blockpos) instanceof IInventory)) return false;
            return true;
        }
    ...
    public void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
        ...
        renderglobal.setupTerrain(entity, partialTicks, icamera, this.frameCount++, Minecraft.thePlayer.isSpectator());
        if (pass == 0 || pass == 2) {
        ...
        TYClient.modManager.getEnableMods().forEach(it -> it.render(partialTicks));
        if (entity.posY + (double)entity.getEyeHeight() >= 128.0) {
    ...
    protected void renderRainSnow(float partialTicks) {
        float f = Minecraft.theWorld.getRainStrength(partialTicks);
        if (f > 0.0f) {
            this.enableLightmap();
            Entity entity = this.mc.getRenderViewEntity();
            WorldClient world = Minecraft.theWorld;
