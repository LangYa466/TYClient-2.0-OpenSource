...
implements INetHandlerPlayClient {
    ...
    public void handleSpawnPlayer(S0CPacketSpawnPlayer packetIn) {
        ...
        EntityOtherPlayerMP entityotherplayermp = new EntityOtherPlayerMP(Minecraft.theWorld, this.getPlayerInfo(packetIn.getPlayer()).getGameProfile());
    ...
    public void handleSpawnMob(S0FPacketSpawnMob packetIn) {
        ...
        float f = (float)(packetIn.getYaw() * 360) / 256.0f;
        float f1 = (float)(packetIn.getPitch() * 360) / 256.0f;
        EntityLivingBase entitylivingbase = (EntityLivingBase)EntityList.createEntityByID(packetIn.getEntityType(), Minecraft.theWorld);
    ...
    public void handleTimeUpdate(S03PacketTimeUpdate packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        Minecraft.theWorld.setTotalWorldTime(packetIn.getTotalWorldTime());
        Minecraft.theWorld.setWorldTime(packetIn.getWorldTime());
    }
    ...
    public void handleSpawnPosition(S05PacketSpawnPosition packetIn) {
        ...
        Minecraft.theWorld.getWorldInfo().setSpawn(packetIn.getSpawnPos());
    }
    ...
    public void handleRespawn(S07PacketRespawn packetIn) {
        ...
        if (packetIn.getDimensionID() != Minecraft.thePlayer.dimension) {
            ...
            this.clientWorldController = new WorldClient(this, new WorldSettings(0L, packetIn.getGameType(), false, Minecraft.theWorld.getWorldInfo().isHardcoreModeEnabled(), packetIn.getWorldType()), packetIn.getDimensionID(), packetIn.getDifficulty(), this.gameController.mcProfiler);
    ...
    public void handleExplosion(S27PacketExplosion packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        Explosion explosion = new Explosion(Minecraft.theWorld, null, packetIn.getX(), packetIn.getY(), packetIn.getZ(), packetIn.getStrength(), packetIn.getAffectedBlockPositions());
        explosion.doExplosionB(true);
        Minecraft.thePlayer.motionX += (double)packetIn.func_149149_c();
        Minecraft.thePlayer.motionY += (double)packetIn.func_149144_d();
        Minecraft.thePlayer.motionZ += (double)packetIn.func_149147_e();
    }
    ...
    public void handleUpdateSign(S33PacketUpdateSign packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        boolean flag = false;
        if (Minecraft.theWorld.isBlockLoaded(packetIn.getPos())) {
            TileEntity tileentity = Minecraft.theWorld.getTileEntity(packetIn.getPos());
            if (tileentity instanceof TileEntitySign) {
    ...
    public void handleBlockAction(S24PacketBlockAction packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        Minecraft.theWorld.addBlockEvent(packetIn.getBlockPosition(), packetIn.getBlockType(), packetIn.getData1(), packetIn.getData2());
    }

    @Override
    public void handleBlockBreakAnim(S25PacketBlockBreakAnim packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        Minecraft.theWorld.sendBlockBreakProgress(packetIn.getBreakerId(), packetIn.getPosition(), packetIn.getProgress());
    }
    ...
    public void handleMaps(S34PacketMaps packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        MapData mapdata = ItemMap.loadMapData(packetIn.getMapId(), Minecraft.theWorld);
    ...
    public void handleEffect(S28PacketEffect packetIn) {
        ...
        if (packetIn.isSoundServerwide()) {
            Minecraft.theWorld.playBroadcastSound(packetIn.getSoundType(), packetIn.getSoundPos(), packetIn.getSoundData());
        } else {
            Minecraft.theWorld.playAuxSFX(packetIn.getSoundType(), packetIn.getSoundPos(), packetIn.getSoundData());
        }
    }
        ...
        for (Map.Entry<StatBase, Integer> entry : packetIn.func_148974_c().entrySet()) {
            ...
            if (statbase.isAchievement() && i > 0) {
                if (this.field_147308_k) {
                    if (Minecraft.thePlayer.getStatFileWriter().readStat(statbase) == 0) {
    ...
    public void handleServerDifficulty(S41PacketServerDifficulty packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        Minecraft.theWorld.getWorldInfo().setDifficulty(packetIn.getDifficulty());
        Minecraft.theWorld.getWorldInfo().setDifficultyLocked(packetIn.isDifficultyLocked());
    }
    ...
    public void handleSoundEffect(S29PacketSoundEffect packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        Minecraft.theWorld.playSound(packetIn.getX(), packetIn.getY(), packetIn.getZ(), packetIn.getSoundName(), packetIn.getVolume(), packetIn.getPitch(), false);
    }
    ...
    public void handleCustomPayload(S3FPacketCustomPayload packetIn) {
        ...
        if ("MC|TrList".equals(packetIn.getChannelName())) {
            ...
            try {
                ...
                if (guiscreen == null || !(guiscreen instanceof GuiMerchant)) return;
                if (i != Minecraft.thePlayer.openContainer.windowId) return;
            ...
            Minecraft.thePlayer.setClientBrand(packetIn.getBufferData().readStringFromBuffer(Short.MAX_VALUE));
            return;
        } else {
            if (!"MC|BOpen".equals(packetIn.getChannelName())) return;
            ItemStack itemstack = Minecraft.thePlayer.getCurrentEquippedItem();
            if (itemstack == null || itemstack.getItem() != Items.written_book) return;
