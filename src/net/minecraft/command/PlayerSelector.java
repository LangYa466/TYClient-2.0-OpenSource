...
public class PlayerSelector
{
    ...
    public static IChatComponent matchEntitiesToChatComponent(final ICommandSender sender, final String token) {
        final List<Entity> list = matchEntities(sender, token, Entity.class);
        if (list.isEmpty()) {
            return null;
        }
        final List<IChatComponent> list2 = Lists.newArrayList();
    ...
    private static <T extends Entity> boolean isEntityTypeValid(final ICommandSender commandSender, final Map<String, String> params) {
        ...
        if (s != null && !EntityList.isStringValidEntityName(s)) {
            final ChatComponentTranslation chatcomponenttranslation = new ChatComponentTranslation("commands.generic.entity.invalidType", s);
    ...
    private static <T extends Entity> List<T> filterResults(final Map<String, String> params, final Class<? extends T> entityClass, final List<Predicate<Entity>> inputList, final String type, final World worldIn, final BlockPos position) {
        ...
        final Predicate<Entity> predicate = Predicates.and(inputList);
        final Predicate<Entity> predicate2 = Predicates.and(EntitySelectors.selectAnything, predicate);
        if (position != null) {
            ...
            if (!params.containsKey("dx") && !params.containsKey("dy") && !params.containsKey("dz")) {
                ...
                if (flag && flag3 && !flag2) {
                    ...
                    list.addAll((Collection<? extends T>)worldIn.getPlayers((Class<? extends Entity>)entityClass, Predicates.and(predicate2, predicate3)));
                }
                else {
