...
public class Cartesian
{
    public static <T> Iterable<T[]> cartesianProduct(final Class<T> clazz, final Iterable<? extends Iterable<? extends T>> sets) {
        return new Product<T>(clazz, toArray(Iterable.class, (Iterable<? extends Iterable>)sets));
    }
    ...
    private static <T> Iterable<List<T>> arraysAsLists(final Iterable<Object[]> arrays) {
        return Iterables.transform(arrays, (Function<? super Object[], ? extends List<T>>)new GetList());
    }
    ...
    static class Product<T> implements Iterable<T[]>
    {
        ...
        public Iterator<T[]> iterator() {
            return (Iterator<T[]>)((this.iterables.length <= 0) ? Collections.singletonList(createArray(this.clazz, 0)).iterator() : new ProductIterator(this.clazz, this.iterables));
        }
        ...
        static class ProductIterator<T> extends UnmodifiableIterator<T[]>
        {
            ...
            private ProductIterator(final Class<T> clazz, final Iterable<? extends T>[] iterables) {
                this.index = -2;
                this.iterables = iterables;
                this.iterators = createArray(Iterator.class, this.iterables.length);
